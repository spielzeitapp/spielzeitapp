-- sync_notification_jobs_for_event: team_notification_settings wie die App persistiert lesen.
-- Vorher: COALESCE(kurzspalte, reminder_spalte) — bei Booleans liefert COALESCE(false, true) = false,
-- obwohl match_second_reminder_enabled = true gespeichert war (stale Kurzspalte oder nur Reminder-Spalten befüllt).
-- Jetzt: nur offizielle Migration-Spalten *_reminder_* (20260323120000).
