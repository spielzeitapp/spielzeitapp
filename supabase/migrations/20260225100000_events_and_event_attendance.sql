-- Events (Spiele, Trainings, sonstige Termine) – einheitliches Datenmodell für Spielplan
-- event_attendance für Teilnahme (opt_in/opt_out) – später für Trainings nutzbar
...
-- Trigger updated_at für events (falls set_updated_at schon existiert)
...
-- RLS event_attendance (analog availability: Team lesen, Trainer/Admin schreiben, Parent eigene Kinder)
...
-- Parent: nur eigene Kinder (player_guardians)
...
CREATE POLICY event_attendance_insert_parent ON public.event_attendance
  ...
  WITH CHECK (
    ...
    AND EXISTS (
      SELECT 1 FROM public.events e
      JOIN public.memberships ms ON ms.team_season_id = e.team_season_id
      WHERE e.id = event_attendance.event_id AND ms.user_id = auth.uid() AND ms.role = 'parent'
    )
  );
