-- Statistik: fehlende event_attendance bei vergangenen Trainings nicht mehr als „dabei“ zählen.
-- Entspricht resolveTrainingAttendanceStatusForStats im Client (nur yes/no/external_training wertbar).
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      END IF;
      -- injured, maybe, leer/fehlend: nicht im Nenner, nicht wertbar
    END LOOP;
...
COMMENT ON FUNCTION public.get_team_training_participation_pct(uuid) IS
  'Ø Beteiligung gewerteter Spieler (teamRatePct yes/(yes+no), ≥30 % Basis). Nur explizit erfasste Status; fehlende Zeilen zählen nicht.';
