-- Pro Spiel: automatischen Matchday-Feed-Post optional deaktivieren (Lineup/Live/Ergebnis unverändert).

ALTER TABLE public.matches
  ADD COLUMN IF NOT EXISTS auto_matchday_feed_enabled boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.matches.auto_matchday_feed_enabled IS
  'false = kein automatischer Matchday-Feed-Post; Lineup/Live/Ergebnis bleiben aktiv.';

-- ensure_matchday_automation: deaktivierte Spiele überspringen
...
BEGIN
  ...
  WHERE e.team_season_id = p_team_season_id
    ...
    AND (
      e.match_id IS NULL
      OR trim(e.match_id::text) = ''
      OR NOT EXISTS (
        SELECT 1 FROM public.matches m
        WHERE m.id = e.match_id
          AND COALESCE(m.auto_matchday_feed_enabled, true) = false
      )
    )
  ORDER BY
    CASE
...
COMMENT ON FUNCTION public.ensure_matchday_automation IS
  'Matchday-Feed-Post (idempotent); überspringt Spiele mit auto_matchday_feed_enabled = false.';
