...
assert.ok(api.includes('https://spielzeitapp.at'));
assert.ok(api.includes('acbaecjzoabafbsjrzvr'));
...
assert.ok(api.includes("origin: LIVE_ORIGIN"));
...
assert.ok(api.includes('parent_invite_origin_ref_mismatch'));
...
assert.ok(inviteLib.includes('parent_invite_origin_ref_mismatch'));
assert.ok(inviteLib.includes('App-Adresse und Datenbank passen nicht zusammen.'));
assert.ok(!inviteLib.includes('Versand nur mit Staging-Datenbank erlaubt.'));
assert.ok(!inviteLib.includes('Versand nur auf Staging erlaubt.'));
