...
// Test identities (keine echten User-IDs)
const NSG = 'club-nsg';
const USC = 'club-usc';
const TS_NSG_U12 = 'ts-nsg-u12';
const TS_NSG_U14 = 'ts-nsg-u14';
const TS_USC_U13 = 'ts-usc-u13';

const clubs = {
  [NSG]: [TS_NSG_U12, TS_NSG_U14],
  [USC]: [TS_USC_U13],
};
...
// Rolle: Trainer NSG → nur NSG-U12
const trainerNsg = [{ clubId: NSG, teamSeasonId: TS_NSG_U12, role: 'trainer' }];
assert.deepStrictEqual(
  listVisibleClubTeamSeasonIds({ clubId: NSG, clubs, memberships: trainerNsg, isPlatformAdmin: false }),
  [TS_NSG_U12],
);
assert.deepStrictEqual(
  listVisibleClubTeamSeasonIds({ clubId: USC, clubs, memberships: trainerNsg, isPlatformAdmin: false }),
  [],
);
assert.deepStrictEqual(
  sessionTeamSeasonIds(trainerNsg, false, [...clubs[NSG], ...clubs[USC]]),
  [TS_NSG_U12],
);

// Vereinsadmin NSG → alle NSG, kein USC
const clubAdminNsg = [{ clubId: NSG, teamSeasonId: TS_NSG_U12, role: 'admin' }];
assert.deepStrictEqual(
  listVisibleClubTeamSeasonIds({ clubId: NSG, clubs, memberships: clubAdminNsg, isPlatformAdmin: false }),
  [TS_NSG_U12, TS_NSG_U14],
);
assert.deepStrictEqual(
  listVisibleClubTeamSeasonIds({ clubId: USC, clubs, memberships: clubAdminNsg, isPlatformAdmin: false }),
  [],
);

// Plattformadmin → NSG + USC
const allSeasons = [...clubs[NSG], ...clubs[USC]];
assert.deepStrictEqual(
  listVisibleClubTeamSeasonIds({ clubId: NSG, clubs, memberships: [], isPlatformAdmin: true }),
  clubs[NSG],
);
assert.deepStrictEqual(
  listVisibleClubTeamSeasonIds({ clubId: USC, clubs, memberships: [], isPlatformAdmin: true }),
  clubs[USC],
);
...
const applyScript = read('scripts/apply-grant-candidates-active-field.mjs');
