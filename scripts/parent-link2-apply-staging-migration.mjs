/**
 * Apply PARENT-LINK.2 role-cast fix to Staging only (acbaecjzoabafbsjrzvr).
 */
import fs from 'fs';
...
const VERSION = '20260812140000';
const sqlPath = join(root, 'supabase/migrations/20260812140000_fix_parent_link_role_cast.sql');
...
const verify = runQuery(
  `SELECT position('role::text' in pg_get_functiondef(p.oid)) > 0 AS has_cast FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname='link_parent_self_service';`,
);
