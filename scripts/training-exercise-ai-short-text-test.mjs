...
assert.match(edge, /setup: \{ type: 'string', maxLength: 110 \}/);
...
assert.match(edge, /Aufbau, Ablauf und Variationen passen nicht vollständig in 700 Zeichen/);
assert.match(edge, /Aufbau und Ablauf um mindestens \$\{contentLength - LIMITS\.content \+ 15\} Zeichen kürzen/);
assert.match(edge, /variationCount: variations\.length/);
