 ...
 *
 * Supported:
 * - https://go.tournament-live.com/38331
 * - https://mobile.tournament-live.com/<page>/<alias>
 * - https://tournament-live.com/...
 * - https://turnier.live/<id> (same vendor, defensive)
 */
...
export type TournamentLiveIdSource = 'path' | 'query' | 'redirect' | 'alias';

export type TournamentLiveUrlResolution = {
  originalUrl: string;
  ...
  alias: string | null;
  idSource: TournamentLiveIdSource | null;
};

const TOURNAMENT_LIVE_HOST_RE =
  /(^|\.)tournament-live\.com$|(^|\.)turnier\.live$|(^|\.)tournamentlive\.com$/i;

export function normalizeTournamentLiveUrl(url: string): string {
...
export function isTournamentLiveHost(url: string): boolean {
...
export function isPlausibleTournamentLiveKey(value: string): boolean {
  const id = value.trim();
...
export function extractTournamentLiveKeyFromUrl(rawUrl: string): {
  id: string | null;
  pageSlug: string | null;
  alias: string | null;
  source: TournamentLiveIdSource | null;
  normalizedUrl: string;
} {
  ...
  try {
    ...
    const meaningful = segments.filter((segment) => !viewSuffixes.has(segment.trim().toLowerCase()));
...
export function labelTournamentLiveIdSource(source: TournamentLiveIdSource | null | undefined): string {
  if (!source) return '—';
  switch (source) {
    case 'path':
      return 'Pfad';
