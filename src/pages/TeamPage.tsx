...
function PlayerBirthPrivacyLines({
  viewerRole,
  birthdate,
}: {
  ...
  const { geburtLabel, alterLabel } = getPlayerBirthDisplayLines(viewerRole ?? "", birthdate);
  if (!geburtLabel && !alterLabel) return null;
  return (
    <div className="mt-1 space-y-0.5 text-xs text-[var(--muted)]">
      {geburtLabel ? <p>{geburtLabel}</p> : null}
      {alterLabel ? <p>{alterLabel}</p> : null}
    </div>
  );
}
          ...
          {teamSeasonId != null && showForm && (
            <form onSubmit={handleFormSubmit} className="mb-3 space-y-2 rounded border border-[var(--border)] bg-[var(--bg)]/50 p-3">
              {mode === "edit" && editingId != null && (
                <PlayerBirthPrivacyLines
                  viewerRole={role}
                  birthdate={players.find((pl) => pl.id === editingId)?.birthdate ?? null}
                />
              )}
            ...
            <ul className="mt-3 space-y-2.5 pb-32">
              {sortedPlayers.map((p) => (
                <li key={p.id}>
                  ...
                  />
                  <PlayerBirthPrivacyLines viewerRole={role} birthdate={p.birthdate} />
                </li>
              ))}
            </ul>
          )}
