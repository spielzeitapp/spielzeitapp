Warning: truncated output (original token count: 55812)
...
export const EventDetailPage: React.FC = () => {
  ...
  const { teamSeasons } = useActiveTeamSeason();
  const { user: sessionUser, isViewOnlyPlayer, memberships, backendRole } = useSession();
  const eventMembershipRole = normalizeRole(
    memberships.find((membership) => membership.team_season_id === event?.team_season_id)?.role ?? null,
  );
  ...
  const eventTeamSeason = teamSeasons.find((teamSeason) => teamSeason.id === event?.team_season_id);
  const ourTeamName = isDemo && demo
    ? demo.data.teamName
    : eventTeamSeason?.team?.name ?? getOurTeamDisplayName();
  ...
  const handleEditSubmit = useCallback(async (ev: React.FormEvent) => {
    ...
    if (editEvent.kind === 'tournament') {
      ...
      const end = (editEndTime ?? '').trim();
      const details = (editDetails ?? '').trim();
      if (t…25812 tokens truncated…   place: linkedVenue.name,
        address: [
          linkedVenue.address,
  ...
  const EventDetailIcon =
    eventTypeRaw.includes('film') || eventTypeRaw.includes('kino')
      ? Clapperboard
      : eventTypeRaw.includes('eltern')
        ? Users
        : eventTypeRaw.includes('abschluss') || eventTypeRaw.includes('fest') || eventTypeRaw.includes('feier')
          ? PartyPopper
          : eventTypeRaw.includes('essen') || eventTypeRaw.includes('pizza')
            ? Pizza
            : eventTypeRaw.includes('ausflug') || eventTypeRaw.includes('fahrt') || eventTypeRaw.includes('bus')
              ? Bus
              : eventTypeRaw.includes('besprech') || eventTypeRaw.includes('meeting')
                ? ClipboardList
                : CalendarPlus;
      ...
      >
        ...
        {event.kind === 'match' && canTrainerManageEvent && (
          <Card className="mt-6 flex flex-col gap-2 overflow-hidden">
            ...
            {feedSectionExpanded ? (
              <div className="pt-1">
                <EventFeedCommunicationSection event={event} userId={sessionUser?.id ?? null} embedded />
              </div>
            ) : null}
          </Card>
        )}
