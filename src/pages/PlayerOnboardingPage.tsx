...
export const PlayerOnboardingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      console.log('[PLAYER ONBOARDING AUTH USER]', { user, authError });
      ...
      console.log('[PLAYER TEAM LOAD START]');
  ...
  useEffect(() => {
    ...
    async function loadPlayersForTeam(teamSeasonId: string) {
      ...
      console.log('[PLAYER PLAYER LOAD RAW]', { data, error });
    ...
    loadPlayersForTeam(selectedTeamSeasonId).catch((e) => {
      console.log('[PLAYER ONBOARDING PLAYER LOAD ERROR]', e);
      if (!alive) return;
      setPlayers([]);
  ...
  const handleSave = async () => {
    if (!userId || !selectedTeamSeasonId || !selectedPlayerId) {
      const msg = 'Bitte Team und Spieler auswählen.';
      console.log('[PLAYER ONBOARDING SAVE ERROR]', msg, {
        userId,
        selectedTeamSeasonId,
        selectedPlayerId,
      });
    ...
    console.log('[PLAYER ONBOARDING TEAM SELECTED]', { teamSeasonId: selectedTeamSeasonId });
    console.log('[PLAYER ONBOARDING PLAYER SELECTED]', { playerId: selectedPlayerId });

    setSaving(true);
    setError(null);

    try {
      // team_id anhand der ausgewählten team_season_id nachladen
      ...
      if (tsError || !tsRow?.team_id) {
        const msg = tsError?.message ?? 'Team konnte nicht ermittelt werden.';
        console.log('[PLAYER ONBOARDING TEAM_SEASON LOOKUP ERROR]', msg);
        setError(msg);
        setSaving(false);
        return;
      }
      ...
      const playerName = selectedPlayer?.display_name ?? null;

      // Duplikatprüfung: pending join_request für (user_id, team_id, 'player')?
      ...
      if (checkError) {
        console.warn('[PLAYER ONBOARDING JOIN_REQUEST CHECK ERROR]', checkError);
      }
      ...
      if (!existingReq) {
        ...
        if (jrError) {
          console.warn('[PLAYER ONBOARDING JOIN_REQUEST INSERT ERROR]', jrError);
          setError(jrError.message ?? 'Spieler-Anfrage konnte nicht gespeichert werden.');
          setSaving(false);
          return;
        }

        console.log('[PLAYER ONBOARDING JOIN_REQUEST CREATED]', {
          user_id: userId,
          team_id: teamId,
          player_name: playerName,
        });
      } else {
        console.log('[PLAYER ONBOARDING JOIN_REQUEST ALREADY EXISTS]', existingReq);
      }
      ...
      <div className="mx-auto max-w-[720px]">
        <Card>
          <div className="space-y-4">
            <CardTitle>Spieler verknüpfen</CardTitle>
            <p className="text-sm text-[var(--text-sub)]">
              Bitte wähle dein Team und deinen Spieler aus dem Kader.
            </p>
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <Button
                    ...
                    className="flex-1"
                    onClick={() => {
                      console.log('[PLAYER ONBOARDING RETRY]');
