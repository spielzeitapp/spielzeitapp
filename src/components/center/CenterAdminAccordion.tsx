...
export function CenterAdminAccordion({ children, defaultExpanded = false }: Props) {
    ...
    <section className={EC_CARD}>
      <div className={EC_CARD_INNER}>
        ...
        >
          ...
          <ChevronDown
            className={`h-3.5 w-3.5 shrink-0 text-white/45 transition-transform duration-200 ${expanded ? 'rotate-180' : ''}`}
            strokeWidth={2.25}
            aria-hidden
          />
        </button>
