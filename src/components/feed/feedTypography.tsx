...
export const FEED_PREMIUM_BADGE_CLASS =
  'sz-club-feed-badge inline-flex min-h-[26px] shrink-0 items-center justify-center gap-1.5 rounded-full border bg-black/40 px-3 py-1 text-[10px] font-semibold leading-none sm:min-h-[28px] sm:text-[11px]';
...
export function FeedSectionHeader({
  ...
  return (
    <div className="sz-club-divider mb-2.5 flex items-center gap-2 border-b pb-2">
      {icon ? (
        <span className="text-[11px] leading-none opacity-90" aria-hidden>
          {icon}
        </span>
      ) : null}
      <p className="sz-club-feed-accent-text text-[10px] font-black uppercase tracking-[0.16em] sm:text-[11px]">{label}</p>
      <div className="sz-club-feed-accent-fade h-px flex-1" aria-hidden />
    </div>
  );
}
...
export const FEED_STADIUM_HERO_SHELL_CLASS =
  'sz-club-feed-shell relative min-w-0 overflow-hidden rounded-none border-y px-2 pb-4 pt-3 sm:rounded-[20px] sm:border sm:px-2.5 sm:pb-4 sm:pt-3.5';
...
export function FeedStadiumHeroBackdrop() {
  return (
    <div className="pointer-events-none absolute inset-0" aria-hidden>
      ...
      />
      <div className="sz-club-feed-poster-background absolute inset-0" />
...
export const FEED_CAPTION_TOGGLE_CLASS =
  'sz-club-feed-accent-text mt-1.5 touch-manipulation text-left text-[13px] font-semibold transition-opacity hover:opacity-100 active:opacity-80';
...
function renderCaptionInline(text: string): React.ReactNode {
  ...
  return (
    <>
      ...
      <span className="sz-club-feed-text-gradient font-extrabold tracking-wide">
        {FEED_HASHTAG}
      </span>
    </>
  );
    ...
    >
      <button
        ...
        className={`${FEED_ACTION_BUTTON_CLASS} ${
          liked ? 'sz-club-feed-accent-text' : 'text-white/62 hover:bg-white/[0.06] hover:text-white/90'
        }`}
        aria-pressed={liked}
      >
