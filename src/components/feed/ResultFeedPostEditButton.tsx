...
import { updateResultFeedPostMedia } from '../../lib/updateResultFeedPostMedia';
...
export const ResultFeedPostEditButton: React.FC<Props> = ({ post, onUpdated }) => {
  ...
  const runUpdate = async (remove: boolean) => {
    ...
    setError(null);
    try {
      const result = await updateResultFeedPostMedia({ post, file, remove });
      if (result.error) {
  ...
  return (
    <>
      <button
        ...
        aria-label="Siegerbild bearbeiten"
        title="Siegerbild bearbeiten"
      >
        ...
        <div className="fixed inset-0 z-[140] flex items-end justify-center bg-black/80 p-0 backdrop-blur-sm sm:items-center sm:p-5">
          <div className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-[28px] border border-red-500/30 bg-[#100d11] p-5 shadow-2xl sm:rounded-[28px]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.22em] text-red-400">Ergebnis-Autopost</p>
                <h2 className="mt-1 text-2xl font-black text-white">Siegerbild bearbeiten</h2>
                <p className="mt-2 text-sm leading-relaxed text-white/60">
                  Das Bild ersetzt die automatische Ergebnisgrafik. Ergebnis, Torschützen und Beitrag bleiben gespeichert.
                </p>
              </div>
            ...
            >
              ...
              <span className="min-w-0">
                <span className="block font-bold">{hasCustomImage ? 'Siegerbild austauschen' : 'Siegerbild hochladen'}</span>
            ...
            {hasCustomImage ? (
              ...
              >
                ...
                Automatische Ergebnisgrafik wiederherstellen
              </button>
            ) : null}
