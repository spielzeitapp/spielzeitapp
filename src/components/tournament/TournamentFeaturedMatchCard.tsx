    ...
    <section className={`${TC_CARD} ${isLive ? 'border-[rgba(255,71,71,0.32)] shadow-[0_0_28px_rgba(255,71,71,0.12)]' : ''}`}>
      <div className={`${TC_CARD_INNER} flex flex-col gap-2`}>
        <button
          ...
          className="flex w-full flex-col gap-2 text-left touch-manipulation"
        >
          ...
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0 flex-1">
              ...
              <p
                className={`font-bold leading-snug text-white break-words ${scoreLine ? 'mt-1 text-[16px]' : 'text-[18px]'}`}
              >
        ...
        {showPrepare ? (
          <TournamentPrepareButton matchId={featured.match_id} variant="primary" />
        ) : null}
      </div>
    </section>
  );
