    ...
    <section className={TC_CARD}>
      <div className={TC_CARD_INNER}>
        <button
          ...
          className="flex w-full min-h-[36px] items-center justify-between gap-2 rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-1.5 text-left touch-manipulation transition hover:bg-white/[0.05] active:bg-white/[0.04]"
        >
          <span className="inline-flex min-w-0 items-center gap-1.5">
            <Settings2 className="h-3.5 w-3.5 shrink-0 text-red-400/80" strokeWidth={2.25} aria-hidden />
            <span className="text-[12px] font-semibold text-white/88">Trainer &amp; Verwaltung</span>
          </span>
          <ChevronDown
            className={`h-3.5 w-3.5 shrink-0 text-white/45 transition-transform ${expanded ? 'rotate-180' : ''}`}
            strokeWidth={2.25}
            aria-hidden
          />
        </button>
...
export function TournamentTrainerAdminSection({
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-white/[0.05] bg-white/[0.015]">
      ...
      >
        <span className={TC_SECTION_LABEL}>{title}</span>
        <ChevronDown
          className={`h-3 w-3 shrink-0 text-white/40 transition-transform ${expanded ? 'rotate-180' : ''}`}
          strokeWidth={2.25}
          aria-hidden
        />
      </button>
