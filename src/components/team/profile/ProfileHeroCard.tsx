...
/**
 * Premium-Profil-Banner — stabile Höhe, kein Layout-Shift beim Bildladen.
 */

export type ProfileHeroVariant = "trainer" | "player";

const HERO_HEIGHT_CLASS = "h-[13rem] sm:h-[14rem]";
const FIGURE_SLOT_CLASS =
  "relative z-[2] h-full w-[50%] max-w-[15.5rem] shrink-0 sm:max-w-[19.25rem]";
...
function useStadiumBackgroundUrl(): string {
  ...
  React.useEffect(() => probeProfileHeroStadiumBackground((resolved) => {
...
function StadiumAtmosphere({ photoBgUrl }: { photoBgUrl: string }) {
  return (
    <div className="pointer-events-none absolute inset-0 z-0" aria-hidden>
      ...
      />
      <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.34)_0%,rgba(2,2,4,0.48)_44%,rgba(0,0,0,0.68)_100%)]" />
...
function HeroAvatarInSlot({
  ...
  return (
    <div
      className={`absolute inset-0 flex items-end justify-center pb-1 transition-opacity duration-300 ${
        visible ? "opacity-100" : "pointer-events-none opacity-0"
      }`}
    >
      <div className="relative h-[6.25rem] w-[6.25rem] overflow-hidden rounded-2xl border border-[#8B0D12]/35 bg-gradient-to-br from-[#161616] via-[#0A0A0A] to-black shadow-[0_8px_24px_rgba(0,0,0,0.55)] sm:h-[6.75rem] sm:w-[6.75rem]">
        {showPhoto ? (
          <img
            src={photoSrc!}
            alt=""
...
function HeroCutoutInSlot({
  ...
  return (
    <div className="absolute inset-0 overflow-visible">
      <div
        className="pointer-events-none absolute left-1/2 top-[4%] h-[44%] w-[76%] -translate-x-1/2 bg-[radial-gradient(ellipse_at_center,rgba(255,255,255,0.09)_0%,transparent_74%)] blur-xl"
        aria-hidden
      />
      <img
        src={cutoutSrc}
        alt=""
        className={`absolute right-0 top-0 h-[118%] w-full max-w-none object-contain object-right object-top drop-shadow-[0_18px_40px_rgba(0,0,0,0.58)] transition-opacity duration-300 ${
...
export const ProfileHeroCard: React.FC<Props> = ({
  ...
  return (
    ...
    >
      ...
      <div className={`relative flex ${HERO_HEIGHT_CLASS} items-center justify-between gap-2 px-3 sm:gap-3 sm:px-4`}>
        <HeroTextBlock
        ...
        {isCutoutLayout ? (
          <div className={FIGURE_SLOT_CLASS} aria-hidden={!cutoutSrc}>
            {cutoutSrc ? (
              <HeroCutoutInSlot
                cutoutSrc={cutoutSrc}
          ...
          <div className={`${FIGURE_SLOT_CLASS} flex items-end justify-center pb-1`}>
