...
/**
 * Premium-Profil-Hero — Zielbild 2: ruhige Kartenserie Trainer + Spieler.
 */
...
/** Zielbild 2 — dunkles Stadion, dezentes Flutlicht, keine milchige Fläche */
...
/** TR / Nummer — ruhiges Outline-Wasserzeichen, kein grauer Block */
function HeroPrimaryWatermark({ watermark, variant }: { watermark: string; variant: ProfileHeroVariant }) {
  ...
  const style: React.CSSProperties = {
    fontSize: isNumeric ? "clamp(8.5rem, 48vw, 15.5rem)" : "clamp(8rem, 44vw, 14.5rem)",
    color: "transparent",
    WebkitTextStroke: "1px rgba(255,255,255,0.2)",
    paintOrder: "stroke fill",
    lineHeight: 0.76,
    letterSpacing: isNumeric ? "-0.045em" : "-0.02em",
    fontVariantNumeric: isNumeric ? "tabular-nums" : undefined,
    opacity: isPlayer ? 0.135 : 0.115,
  };
...
/** Zielbild 2 — Trainer: Name → Rolle → Team → Saison | Spieler: Team → Rolle → Name → Saison */
  ...
  if (variant === "player") {
    return (
      <div className="relative z-[4] flex min-w-[40%] max-w-[52%] flex-1 flex-col justify-center gap-1 self-stretch py-3 pl-0.5 sm:min-w-[38%]">
        {teamLine ? (
          <p className={`${roleClass} !text-[#E50914]`}>
        ...
        {seasonLine ? <p className={`mt-0.5 ${seasonClass}`}>{seasonLine}</p> : null}
      </div>
    );
  }
...
/** Freisteller — 88–92 % Hero-Höhe, rechts, dezenter weißer Glow */
